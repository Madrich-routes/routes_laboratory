...
class DistanceClipper(BaseTransformer):
    """Оставляем в матрице только ребра от a_min до a_max. Меньшие ребра клипаются, а большие заменяются на
    бесконечность.

    save_matrix определет хотим ли мы сохранить матрицу, чтобы потом восстановить # TODO: сохранять только
    изменяемые куски
    """
