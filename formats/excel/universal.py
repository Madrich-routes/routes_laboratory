...
1. Заказы:
    ...
    [lat: float, lon: float, address: string, time_windows: List[Tuple[int, int]], amounts: np.ndarray, delay: int, price: int, priority:int]
2. Курьеры
...
class Universal:
    ...
    def to_excel(self, problem: RichVRPProblem, path: str):
        ...
        jobs = pd.DataFrame(jobs_list, columns=['Широта', 'Долгота', 'Адрес', 'Временные рамки', 'Характеристики', 'Время обслуживания', 'Цена', 'Приоритет'])
        agents = pd.DataFrame(agents_list, columns=['Имя', 'График работы', 'Тип', 'Начальная точка', 'Конечная точка'])
        depots = pd.DataFrame(depots_list, columns=['Адрес', 'Широта', 'Долгота', 'График работы', 'Время обслуживания'])
    ...
    def from_excel(self, path: str) -> RichVRPProblem:
        ...
        res = RichVRPProblem(DistanceMatrixGeometry(np.array([]), np.array([]), 0), agents_list, jobs_list, depots_list, [])
        return res
...
def time_windows_to_str(time_windows: List[Tuple[int, int]])->str:
    res = ', '.join([f'({datetime.fromtimestamp(i[0])} - {datetime.fromtimestamp(i[1])})' for i in time_windows])
    # res = ', '.join([f'({str(datetime.fromtimestamp(i[0])).split()[1]} - {str(datetime.fromtimestamp(i[1])).split()[1]})' for i in time_windows])
    return res
'''
...
def str_to_time_windows(raw_string: str)->List[Tuple[int, int]]:
    ...
    time_windows = [(int(datetime.strptime(item[1:-2].split(' - ')[0], '%Y-%m-%d %H:%M:%S').timestamp()),
            int(datetime.strptime(item[1:-2].split(' - ')[1], '%Y-%m-%d %H:%M:%S').timestamp())) for item in sep_strings]
    return time_windows
...
if __name__ == '__main__':
    ...
    costs = AgentCosts()
    agent = Agent(id=0, costs=costs, value=[], time_windows=[(start,end)], start_place=0, end_place=0, type=AgentType(0, 54, 0, [], costs, [], 'Водитель'), name='Алексей')
    ...
    test = Universal()
    test.to_excel(res, 'tst.xlsx')
    test.from_excel('tst.xlsx')
