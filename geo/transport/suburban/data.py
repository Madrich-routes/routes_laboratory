...
def build_df() -> pd.DataFrame:
    ...
    walker = MOWalker(all_uids_file=all_uids_file, all_pathes_file=all_pathes_file)
    return walker.dataframe


class MOWalker:
    """
    То, что есть в яндекс-расписании.
    1. Электрички
    2. Маршрутки в область
    """

    def __init__(self, dataframe=None, all_uids_file=None, all_pathes_file=None):
        """
        parse TAT xml
        """
        if not dataframe:
            self.dataframe = self.parse_tatdata(all_uids_file, all_pathes_file)
        else:
    ...
    def parse_tatdata(
        ...
        if all_pathes_file:
            all_pathes = pkl.load(all_pathes_file)
        else:
            all_pathes = []
