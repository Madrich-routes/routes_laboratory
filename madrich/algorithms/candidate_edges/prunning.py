"""В этом модуле откидываются лишние ребра-кандидаты, а также им назначаются новые приоритеты с учетом
ограничений."""
from typing import List, Set

from madrich.algorithms.candidate_edges.models import CandidateEdge
from madrich.utils.iteration import sets_intersection
...
def prune_time_windows(
    ...
    """Убираем те ребра, которые точно не подходят по временным окнам candidates — Изначальное множество
    кандидатов для каждой точки матрицы.
...
def prune_skills(
    ...
    Тем, которые подходят выставляем приоритеты. (Их вероятность быть успешными)
    """
    pruned_candidates = []
