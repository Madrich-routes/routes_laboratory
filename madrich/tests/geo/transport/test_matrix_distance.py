...
from madrich.geo.transport.calc_distance import transport_travel_time, combined_matrix
...
def check_transport_travel_time(
    ...
    if time == p_matrix[src, dst]:
        assert src_station is None and dst_station is not None, 'Если пешком, то станций нет'
    else:
        assert (
            time == s_matrix[src_station, dst_station]
