...
def get_matrix(points: Union[array, list], factor: Union[str, list], transport: str, dst=None, profile="driving"):
    ...
    if dst is not None:
        polyline, params = _encode_src_dst(points, dst)
    else:
        polyline, params = _encode_src(points)

    params["annotations"] = ",".join(factor) if type(factor) is list else factor

    r = requests.get(f"{host}/table/v1/{profile}/polyline({polyline})?{urlencode(params)}")
