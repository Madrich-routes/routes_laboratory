...
def save_excel(name: str, couriers_path: str, depots_path: str, directory='') -> str:
    """ Generate first excel file with summary info; return file path
    """
    file_name = f'{name}_info.xlsx' if not directory else f'{directory}/{name}_info.xlsx'
...
def save_couriers(name: str, couriers_directory: str, directory='') -> str:
    ...
    """
    file_name = f'{name}_couriers.xlsx' if not directory else f'{directory}/{name}_couriers.xlsx'
    writer = pd.ExcelWriter(file_name)
    for file in os.listdir(couriers_directory):
        ...
        couriers.to_excel(writer, sheet_name=file.replace('Мск,', '').replace(' ', '')[:31], index=False)
    writer.save()
    return file_name
