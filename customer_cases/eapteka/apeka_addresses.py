...
def main():
    ...
    result_df = pd.DataFrame(res,
                             columns=['Адрес из Excel', 'Коммент из Excel', 'Адрес из геокодера', 'Долгота', 'Широта'])
    result_df.to_excel("eapteka_result.xlsx")
    i = 0


def get_coords(address):
    map_request = "http://search.maps.sputnik.ru/search/addr?q={address})".format(address=address.replace(" ", "%20"))
    response = requests.get(map_request)
    content = json.loads(response.content)
    geocodeAddress, lot, lan = '', '', ''
    if len(content["result"]) > 1:
        coord = content["result"]["address"][0]["features"][0]["geometry"]["geometries"][0]["coordinates"]
        lot = str(coord[1])
        lan = str(coord[0])
        geocodeAddress = content["result"]["address"][0]["features"][0]['properties']['display_name']
