...
for i in range(rows_count):
    ...
    res.append([s_date, e_date, s_address, e_address, s_coord, e_coord, s_matr_id, e_matr_id, linear_dist, matrix_dist, car])
    print(f"Обработали уже {i} из {rows_count}")

result_df = pd.DataFrame(res, columns=['Время погрузки', 'Время разгрузки', 'Адрес погрузки', 'Адрес разгрузки',
                                       'Координаты погрузки', 'Координаты разгрузки', "s_id", "e_id", 'Расстояние по прямой, м',
                                       'Расстояние по матрице, м', 'Машина'])
